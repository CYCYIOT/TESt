...
//ps_info_s gps;
    double gps_data[4]={0};
//	gps_get_info(&gps);
	get_gps_uart_data(gps_data);
//ata.lat = gps.lat;
//ata.lon = gps.lon;	
//ata.eph = gps.eph;
//ata.satellites = gps.satellites_used;
//ata.fix_type = gps.fix_type;

    data.satellites = gps_data[0];
    data.lon =gps_data[1];
    data.lat =gps_data[2];
	data.fix_type = 3;
	data.eph = gps_data[3];
...
void awlink_encode_status_user1_info(awlink_s * link)
{
	...
	get_ultrasonic_data(ul_data);
	data.data1 = ul_data[0];
	data.data2 = ul_data[1];
	data.data3 = ul_data[2];
	data.data4 = ul_data[3];
	data.data5 = 0;
