...
void control_poshold_update(float dt,rc_s * rc)
{
    ...
    if(control_get_mode_last() == CONTROL_MODE_THROWN && get_thrown_flag()== true){      //抛飞完成后定高一次
     if(count_th++ > 5000){
	 set_tof_althold(100); 
	 set_thrown_flag(false);
	 count_th=0;
     }
	}
		...
		case STATUS_MOVE: 
			...
			if(fabs(rc->roll) == 0 && fabs(rc->pitch) == 0){
				...
				if(poshold_move_break_time_check >= 0.1f){
					if(flow_get_quality_f() > 0.3f){           //0.3f
						status_next = STATUS_BREAK2;
					}else{
