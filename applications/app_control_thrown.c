...
void set_thrown_flag(bool vel)
{
  thrown_flag = vel;
}

bool get_thrown_flag()
{
  return thrown_flag;
}
...
void control_thrown_update(float dt,rc_s * rc)
{
	...
	if(takeoff_step == TAKEOFF_STEP_SPIN){
        ...
        debug_t("pos.vel_ned[0]=%f pos.vel_ned[1]=%f pos.vel_ned[2]=%f\n,",pos.vel_ned[0],pos.vel_ned[1],pos.vel_ned[2]);
		
		if(pos.vel_ned[2] < -0.6 || pos.vel_ned[2] > 0.5 || pos.vel_ned[1] < -0.8 || pos.vel_ned[1] > 0.5 || pos.vel_ned[0] < -0.8 || pos.vel_ned[0] > 0.5){
			tkof_pos_sp[2] = pos.pos_ned[2] + takeoff_alt;
		...
		if(fabs(rc->pitch) > 0 || fabs(rc->roll) > 0){
			...
			//	control_vel_ned_xy_common(dt,vel_sp,0,0);
	...
	//	control_alt_pos_common(dt,tkof_pos_sp[2],takeoff_vel_limit);
		control_alt_pos_common(dt,tkof_pos_sp[2],10.0f);

		nav_att_offset_update();		

		if(fabs(rc->thr_raw) > takeoff_rc_check || fabs(pos.pos_ned[2] - tkof_pos_sp[2]) < 0.10f || get_tof_data_yaw() > 1.0f ){
